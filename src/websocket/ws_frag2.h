...
typedef union 
{
	...
	uint16_t len;
} ws_frag_len_126_t __attribute__((packed));

typedef struct
{
	ws_frag_len_126_t len;
	unsigned char mask[4];
} ws_frag_len_126_masked_t __attribute__((packed));

typedef union
{
	unsigned char bytes[8];
	uint64_t len;
} ws_frag_len_127_t __attribute__((packed));

typedef struct
{
	ws_frag_len_127_t len;
	unsigned char mask[4];
} ws_frag_len_127_masked_t  __attribute__((packed));
...
typedef struct
{
	...
	union {
		ws_frag_len_126_masked_t for126;
		ws_frag_len_127_masked_t for127;
	} len;
} ws_frag_preamble_t;
